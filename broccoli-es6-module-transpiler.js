...
ES6ModuleTranspiler.prototype.write = function (readTree, destDir) {
  ...
  return readTree(this.inputTree).then(function (srcDir) {
    ...
    destDir = path.join(destDir, self.inputTree.options.destDir)
    var inputFiles = helpers.multiGlob(self.files, {cwd: path.join(srcDir, self.inputTree.options.destDir)})
    ...
    function addModule (moduleName) {
      ...
      var fullPath = path.join(srcDir, self.inputTree.options.destDir, modulePath)
      var imports
      try {
        ...
        mkdirp.sync(path.join(destDir, path.dirname(moduleName)))
        fs.writeFileSync(path.join(destDir, moduleName + ".js"), compiledModule)
      } catch (err) {
      ...
      for (i = 0; i < imports.length; i++) {
        ...
        addModule(self.resolver(importName))
      }
    }
  })
}
